    ...
    for roots, dirs, files in os.walk(path_predict):
        if files:
            for file in files:
                ...
                unite = np.sum(img_pre)+np.sum(img_true)+0.000001
                TP = TP+inter
                FPN = FPN + unite
                ds.append(2*inter/unite)
                single_I=inter
                single_U=unite-single_I
                precisions_arr.append(inter/(np.sum(img_pre)+0.0000001))
                recall_arr.append(inter/np.sum(img_true))
                Jaccard.append(single_I/single_U)
                sensitivity = inter/np.sum(img_true)
