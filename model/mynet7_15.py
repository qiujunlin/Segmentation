...
class COM(nn.Module):
    def __init__(self, channel):
        ...
        self.conv_upsample1_4 = BasicConv2d(channel, channel, 3, padding=1)
        self.conv_upsample1_3 = BasicConv2d(channel, channel, 3, padding=1)
        self.conv_upsample1_2 = BasicConv2d(channel, channel, 3, padding=1)
        self.conv_upsample2_4 = BasicConv2d(channel, channel, 3, padding=1)
        self.conv_upsample2_3 = BasicConv2d(channel, channel, 3, padding=1)
        self.conv_upsample3_4 = BasicConv2d(channel, channel, 3, padding=1)
        ...
        self.conv_concat2 = BasicConv2d(3 * channel, 3 * channel, 3, padding=1)
        self.conv_concat1 = BasicConv2d(4 * channel, 4 * channel, 3, padding=1)  # 最大 64*4 = 256 不大
        ...
        self.out2_1 = BasicConv2d(channel * 4, 1, 1)
        self.out2_2 = BasicConv2d(channel * 3, 1, 1)
        self.out2_3 = BasicConv2d(channel * 2, 1, 1)
        self.out2_4 = BasicConv2d(channel, 1, 1)
    ...
    def forward(self, x1, x2, x3,x4):
        ...
        x1_1 = self.conv_upsample1_4(self.upsample(self.upsample(self.upsample(x4)))) \
               * self.conv_upsample1_3(self.upsample(self.upsample(x3))) \
               * self.conv_upsample1_2(self.upsample(x2)) * x1

        x2_1 = self.conv_upsample2_4(self.upsample(self.upsample(x4))) \
               * self.conv_upsample2_3(self.upsample(x3)) * x2

        x3_1 = self.conv_upsample3_4(self.upsample(x4)) * x3

        x4_1 = x4

        x3_2 = torch.cat((x3_1, self.conv_upsample4(self.upsample(x4_1))), 1)
        x3_2 = self.conv_concat3(x3_2)

        x2_2 = torch.cat((x2_1, self.conv_upsample5(self.upsample(x3_2))), 1)
        x2_2 = self.conv_concat2(x2_2)

        x1_2 = torch.cat((x1_1, self.conv_upsample6(self.upsample(x2_2))), 1)
        x1_2 = self.conv_concat1(x1_2)
        x1 = self.conv4(x1_2)
...
class MyNet(nn.Module):
    def __init__(self, channel=32):
       ...
       # self.decoder5 = DecoderBlock(in_channels=512, out_channels=512)
        ...
        self.edge_conv4 =BasicConv2d(512,channel,1)
        self.edge_conv3 =BasicConv2d(320,channel,1)
        self.edge_conv2 =BasicConv2d(128,channel,1)
        self.edge_conv1 =BasicConv2d(64,channel,1)
        self.edge_out1 =nn.Conv2d(channel,1,1)
        self.edge_out2 =nn.Conv2d(channel,1,1)
        self.edge_out3 =nn.Conv2d(channel,1,1)
        self.edge_out4 =nn.Conv2d(channel,1,1)
        self.down = BasicConv2d(channel*4,channel,1)
        self.atte = BCA(channel,channel,channel)

        self.trans =BasicConv2d(channel,channel,1)
    ...
    def forward(self, x):
        ...
        r1 =self.Translayer1(x1)
        r2 =self.Translayer2(x2)
        r3 =self.Translayer3(x3)
        r4 =self.Translayer4(x4)

        # MSB
        e1 = self.edge_conv1(x1)
        e1 = F.interpolate(e1, x4.size()[2:], mode='bilinear', align_corners=True)
        e1_out = self.edge_out1(e1)
        e2 = self.edge_conv2(x2)
        e2 = F.interpolate(e2, x4.size()[2:], mode='bilinear', align_corners=True)
        e2_out = self.edge_out2(e2)
        e3 = self.edge_conv3(x3)
        e3 = F.interpolate(e3, x4.size()[2:], mode='bilinear', align_corners=True)
        e3_out = self.edge_out3(e3)
        e4 = self.edge_conv4(x4)
        e4 = F.interpolate(e4, x4.size()[2:], mode='bilinear', align_corners=True)
        e4_out = self.edge_out4(e4)
        e = torch.cat((e1, e2, e3, e4), dim=1)
        e = self.down(e)
        bound_out_ = e1_out+e2_out+e3_out+e4_out

        pred1 = self.COM(r1,r2,r3,r4)

        preout1 =self.unetout2(pred1)

        att1 =self.down03(pred1)
        pred2= self.atte(att1,e)

        preout2 =self.unetout2(pred2)

        preout2 = F.interpolate(preout2,scale_factor=32,mode='bilinear')
        preout1 = F.interpolate(preout1, scale_factor=4, mode='bilinear')
        bound_out_ = F.interpolate(bound_out_, scale_factor=32, mode='bilinear')

        return preout1,preout2,bound_out_
...
if __name__ == '__main__':
    ...
    pred2,pred1,bound= model(input_tensor)
    print(pred2.size())
    print(pred1.size())
    print(bound.size())
