...
class BCA(nn.Module):
    def __init__(self, xin_channels, yin_channels, mid_channels, BatchNorm=nn.BatchNorm2d, scale=False):
        ...
        self.at = LM(xin_channels)


    def forward(self, x, y):
        z = torch.sigmoid(y) * x +x
        z =self.conv1(z)
        z =self.ca(z)
        z =self.sa(z)
...
class LM(nn.Module):
    def __init__(self, channel):
        ...
        # local
        self.local1 = BasicConv2d(in_planes=channel,out_planes=channel,kernel_size=3,stride=1,padding=1)
        self.local2 = BasicConv2d(in_planes=channel,out_planes=channel,kernel_size=3,stride=1,padding=2,dilation=2)
        self.local3 = BasicConv2d(in_planes=channel,out_planes=channel,kernel_size=3,stride=1,padding=3,dilation=3)

        self.conv_cat = BasicConv2d(in_planes=channel*3,out_planes=channel,kernel_size=3,stride=1,padding=1)
...
class CAM(nn.Module):
    ...
    def forward(self, x_high, x_low):
        ...
        right_1 = self.down(x_low, inplace=True)
        right_2 = x_low
...
class MyNet8(nn.Module):
    ...
    def __init__(self, channel=64,numclass=1):
        ...
        self.combine1_1 = CAM(channel)
        self.combine2_1 = CAM(channel)
        self.combine3_1 = CAM(channel)
    ...
    def forward(self, x):
      ...
      #  x4 = self.aspp(x4)
       ...
       # d4 = self.decoder_s4(x4)     # bs, 1024, 11, 11
        ...
        d3 =self.combine3_1(x4,x3_1)       # bs, 1024, 11, 11
      ...
      #  d3=self.decoder_s3(d3)       # bs, 1024, 22, 22
        ...
        return  self.upsample3(boundary3), self.upsample2(boundary2), self.upsample1(boundary1), self.upsample3(segmentation3),  self.upsample2(segmentation2),  self.upsample1(segmentation1)
